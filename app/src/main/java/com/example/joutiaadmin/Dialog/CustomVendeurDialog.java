...
import com.example.joutiaadmin.ProduitListActivity;
...
public class CustomVendeurDialog extends Dialog {

    TextView nomComplet,mail,phone,adresse,ville,categorie;
    Button delete, produit;
    public CustomVendeurDialog(Context context, Object object, List<Vendeur> allproductarray) {
        super(context);
        requestWindowFeature(Window.FEATURE_NO_TITLE);
        setContentView(R.layout.custom_dialog_vendeur);
        ...
        produit = findViewById(R.id.produitUse);
        ...
        produit.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent i = new Intent(getContext(), ProduitListActivity.class);
                i.putExtra("vendeur",nomComplet.getText().toString());
    ...
    private void DeleteHere() {
        ...
        DatabaseReference databaseReference = FirebaseDatabase.getInstance().getReference("vendeur");
    ...
    private void ShowAllInfo(Object object, List<Vendeur> allproductarray) {
        for(int i=0;i<allproductarray.size();i++){
            if(allproductarray.get(i).nommagasin.equals(object)){
                nomComplet.setText(allproductarray.get(i).nommagasin);
                mail.setText(allproductarray.get(i).mail);
                phone.setText("Téléphone : "+allproductarray.get(i).phone);
                adresse.setText("Adresse : "+allproductarray.get(i).adressemagasin);
                ville.setText("Ville : "+allproductarray.get(i).villemagasin);
                categorie.setText(allproductarray.get(i).catmagasin);
            }
        }
    }
